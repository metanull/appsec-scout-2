...
| `redis` | `redis:7-alpine` | Cache and queue backend (sessions live in the primary database) |
...
| `static-analysis-collector` | `appsec-scout-static-analysis-collector:latest` | Isolated worker for in-app static analysis: git + .NET/Roslynator (with the .NET Framework 4.6.2/4.7.2/4.8 targeting packs) + Java/Maven/Gradle/SpotBugs+FindSecBugs + Opengrep, consumes only the `static-analysis` queue — see [docs/concepts/static-analysis-collection.md](concepts/static-analysis-collection.md) |
