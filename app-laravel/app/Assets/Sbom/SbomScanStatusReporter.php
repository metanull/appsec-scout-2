...
/**
 * Read-only status for every sbom-scan run currently on disk, shared by the
 * `sbom:scan-status` command and the Filament Operations page so both report
 * exactly the same thing. Derives everything from artifacts that already exist
 * (run.jsonl, summary.json, the .processed cursor, sbom-import ErrorLog rows) —
 * no new persisted state.
 */
final class SbomScanStatusReporter
{
    ...
    private function statusForRun(string $runDir, string $runName, ?int $approxTotal, int $failed): array
    {
        ...
        $cursorFile = (string) config('sbom.cursor_path') . '/' . $runName . '.processed';
