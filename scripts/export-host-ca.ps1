...
Get-ChildItem -Path $resolvedOutputDir -File -Filter '*.crt' | Remove-Item -Force
...
if ($certificatesByThumbprint.Count -eq 0) {
    throw 'No trusted certificates were found in the local machine or current user Root/CA stores.'
}
