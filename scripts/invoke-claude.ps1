...
try {
    ...
    if ($Rebuild -or -not $imageExists) {
        ...
        if ($Rebuild) {
            Invoke-Docker compose build claude --no-cache
        } else {
            Invoke-Docker compose build claude
        }
    }
    ...
    switch ($Mode) {
        'login' {
            ...
            Invoke-Docker compose run --rm -it --no-deps @envOverrides claude --login
        ...
        'shell' {
            ...
            Invoke-Docker compose run --rm -it --no-deps @envOverrides claude --shell
        }
        'task' {
            ...
            try {
                ...
                Invoke-Docker compose run --rm --no-deps -e CLAUDE_TASK @envOverrides claude
            } finally {
